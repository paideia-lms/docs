...
- **ポート**: ポート3000、3001、5432、9000、9001、4983が利用可能である必要があります
...
本番デプロイまたはインフラストラクチャの完全な制御が必要な場合は、Docker Composeを使用します。このセットアップには、Paideia LMS、PostgreSQLデータベース、MinIO（S3互換ストレージ）、およびDrizzle Gatewayが含まれます。
...
4. サービスが実行中であることを確認：
   ...
   4つのサービス（paideia、postgres、minio、drizzle-gateway）すべてが「Up」ステータスで表示されるはずです。
